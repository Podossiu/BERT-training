...
def make_dir(args, is_large, lr):
    ...
    hyperparam_dir = 'wd%s_ad%s_d%s_lr%s' %(str(args.weight_decay), str(args.attn_dropout), str(args.dropout), str(lr))
...
def arg_parse():
    ...
    parser.add_argument('--mode', type = str, choices = ["eval", "train"])
...
if args.mode == "train":
    ...
    if args.quant_mode == 'symmetric':
        warm_updates = '0' # no warm update for Q.A. finetuning
    ...
    subprocess_args = [
        'python3',    args.mode + ".py",
        '--learning-rate', str(lr),
        '--weight-decay', str(args.weight_decay),
        '--optim', 'adam',
        '--adam-beta1', str(0.9),
        '--adam-beta2', str(0.98),
        '--adam-epsilon', str(1e-06),
        '--lr-scheduler-type', 'polynomial_decay',
        '--max-train-epochs', str(max_epochs),
        ...
        '--clip-norm',  '0.0',
        '--total-num-update', str(total_num_updates), '--warmup-updates', str(warm_updates),
